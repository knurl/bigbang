...
class Tunnel:
    ...
    def __str__(self):
        ...
        return "{l} -> {ra}:{rp} (PID {p})".format(p =
                self.p.pid if self.p != None else "?", l = self.lport,
...
def getSecrets() -> None:
    ...
    try:
        for name, values in s.items():
            ...
            # If the secret is generated by the program, then it will not
            # already be on disk at this point...
            if "generated" in values and values["generated"] == True:
                continue

            # ...otherwise, check we can read the file
